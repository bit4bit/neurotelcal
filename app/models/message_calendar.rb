    ...
    if message
      if message.group.campaign.plivo.all.size > 0
        ...
        if channels > total_channels_plivos
          errors.add(:channels, 'Ha superado el limite de canales del plivo o plivos de la Campaña')
        end
      else
        errors.add(:channels, 'No hay plivos configuarados')
      end
      
      mcls = MessageCalendar.select('channels').where(:message_id => message.id)
      if mcls.all.size > 0
        ...
        if channels + total_channels_messages_calendars > total_channels_plivos
          errors.add(:channels, 'Ha superado entre los calendarios el limite de los plivos de la Campaña')
        end
      end
